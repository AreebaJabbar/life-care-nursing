...
<section class="page-header">
  <div class="page-header-photo">
    <img src="assets/doctor_1.jpg" alt="Our Team">
  </div>
  <div class="page-header-badge"><i class="bi bi-diagram-3-fill"></i></div>
  <div class="container page-header-inner">
    <div class="eyebrow mb-3">Management & Leadership</div>
    <h1>Our Leadership Team</h1>
    <div class="breadcrumb-care"><a href="index.html">Home</a> <i class="bi bi-chevron-right"></i> Our Team</div>
  </div>
</section>

<!-- MAIN TEAM PANEL GRID -->
<section class="py-5" style="background:#FAF8F3;">
  <div class="container py-4">
    <div class="text-center mb-5">
      <h2 class="display-6 font-weight-bold" style="color: var(--teal-900);">Meet Our Dedicated Management Team</h2>
      <p class="text-muted mx-auto" style="max-width: 650px; font-size: 1rem;">
        The experienced healthcare professionals and management team guiding LifeCare Nursing & Medical Services.
      </p>
    </div>

    <div class="row g-4">
      <?php if (empty($team)): ?>
        <div class="col-12 text-center py-5 text-muted">No team cards uploaded yet. Upload cards from Admin Panel.</div>
      <?php else: ?>
        <?php foreach ($team as $tm): ?>
          <?php 
            $waNumber = !empty($tm['whatsapp']) ? preg_replace('/[^0-9]/', '', $tm['whatsapp']) : '923008053198';
            $waMsg = urlencode("Hello, I would like to contact team member " . $tm['name'] . " (" . $tm['role'] . ").");
            $waUrl = "https://wa.me/{$waNumber}?text={$waMsg}";
          ?>
          <div class="col-md-6 col-lg-4">
            <div class="panel-card">
              <div class="panel-card-img-holder">
                <img src="<?= htmlspecialchars($tm['image']) ?>" alt="<?= htmlspecialchars($tm['name']) ?>">
                <span class="panel-card-badge"><?= htmlspecialchars($tm['badge'] ?? 'Management') ?></span>
              </div>
              <div class="panel-card-body">
                <div>
                  <div class="panel-card-name"><?= htmlspecialchars($tm['name']) ?></div>
                  <div class="panel-card-role"><?= htmlspecialchars($tm['role']) ?></div>
                  <p class="panel-card-desc"><?= htmlspecialchars($tm['description']) ?></p>
                </div>
                <a href="<?= $waUrl ?>" target="_blank" class="btn-care panel-card-btn">
                  <i class="bi bi-chat-dots-fill me-1"></i> Contact Member
                </a>
              </div>
            </div>
          </div>
...
<div class="floating-contact-stack">
  ...
  <a href="https://wa.me/923008053198?text=Hello%2C%20I%20would%20like%20to%20get%20in%20touch" target="_blank" class="floating-contact-btn btn-wa">
    <i class="bi bi-whatsapp"></i> <span>Get in Touch</span>
  </a>
</div>
