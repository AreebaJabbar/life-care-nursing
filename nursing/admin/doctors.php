...
if (isset($_GET['action']) && $_GET['action'] === 'status' && isset($_GET['id']) && isset($_GET['status'])) {
    ...
    foreach ($doctors as &$d) {
        if ((int)$d['id'] === $targetId) {
            $d['status'] = $newStatus;
            break;
        }
    }
    saveData($type, $doctors);
    header('Location: doctors.php?msg=status_updated');
    exit;
}
...
if (isset($_GET['msg'])) {
    ...
    if ($_GET['msg'] === 'status_updated') $msg = 'Doctor account status updated successfully!';
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="m-0 text-dark">Doctor Directory & Approval Management</h2>
        <p class="text-muted m-0">Review self-registrations, approve/disable accounts, and manage doctor profiles.</p>
    </div>
    <button type="button" class="btn-care" data-bs-toggle="modal" data-bs-target="#doctorModal" onclick="resetDoctorForm()">
        <i class="bi bi-person-plus-fill me-1"></i> Add Doctor (Admin Backup)
    </button>
</div>
        ...
        <table class="table table-hover align-middle mb-0" style="font-size: 0.9rem;">
            ...
            <tbody>
                <?php if (empty($doctors)): ?>
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">No doctor profiles found.</td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="text-muted" style="font-size: 0.82rem;">
                                    <?php if (!empty($doc['username'])): ?>
                                        <i class="bi bi-person me-1"></i><?= htmlspecialchars($doc['username']) ?> &bull; 
                                    <?php endif; ?>
                                    <?= htmlspecialchars($doc['qualifications'] ?? $doc['badge'] ?? '') ?>
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <?php if ($docStatus !== 'approved'): ?>
                                    <a href="doctors.php?action=status&status=approved&id=<?= $doc['id'] ?>" class="btn btn-sm btn-success me-1" title="Approve Registration">
                                        <i class="bi bi-check-lg"></i>
                                    </a>
                                <?php endif; ?>

                                <?php if ($docStatus !== 'disabled'): ?>
                                    <a href="doctors.php?action=status&status=disabled&id=<?= $doc['id'] ?>" class="btn btn-sm btn-warning me-1" title="Disable Account">
                                ...
                                <a href="doctors.php?action=delete&id=<?= $doc['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this doctor account?');" title="Delete">
