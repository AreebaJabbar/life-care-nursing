...
// Handle Status Change Action (Approve / Disable)
if (isset($_GET['action']) && $_GET['action'] === 'status' && isset($_GET['id']) && isset($_GET['status'])) {
    $targetId = (int)$_GET['id'];
    $newStatus = in_array($_GET['status'], ['approved', 'pending', 'disabled']) ? $_GET['status'] : 'approved';
...
if (isset($_GET['msg'])) {
    ...
    if ($_GET['msg'] === 'status_updated') $msg = 'Staff account status updated successfully!';
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="m-0 text-dark">Staff Directory & Approval Management</h2>
        <p class="text-muted m-0">Review staff self-registrations, approve/disable accounts, and manage nursing staff.</p>
    </div>
    <button type="button" class="btn-care" data-bs-toggle="modal" data-bs-target="#staffModal" onclick="resetStaffForm()">
        <i class="bi bi-person-plus-fill me-1"></i> Add Staff (Admin Backup)
    </button>
</div>
    ...
    <div class="table-responsive p-0">
        <table class="table table-hover align-middle mb-0" style="font-size: 0.9rem;">
            ...
            <tbody>
                <?php if (empty($staff)): ?>
                    <tr>
                        <td colspan="6" class="text-center text-muted py-4">No staff profiles found.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($staff as $stf): 
                        $stfStatus = $stf['status'] ?? 'approved';
                    ?>
                        <tr>
                            <td>
                            ...
                            <td>
                                ...
                                <div class="text-muted" style="font-size: 0.82rem;">
                                    <?php if (!empty($stf['username'])): ?>
                                        <i class="bi bi-person me-1"></i><?= htmlspecialchars($stf['username']) ?> &bull; 
                                    <?php endif; ?>
                                    <?= htmlspecialchars($stf['role']) ?>
                                </div>
                            </td>
                            <td>
                                <span class="badge bg-secondary-subtle text-dark border me-1 px-2 py-1"><i class="bi bi-geo-alt-fill me-1 text-danger"></i> <?= htmlspecialchars($stf['city'] ?? 'Faisalabad') ?></span>
                                <?php if ($stfStatus === 'approved'): ?>
                                ...
                                <?php else: ?>
                                    <span class="badge bg-danger-subtle text-danger border border-danger-subtle px-2 py-1"><i class="bi bi-slash-circle me-1"></i> Disabled</span>
                                <?php endif; ?>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <?php if ($stfStatus !== 'approved'): ?>
                                    <a href="staff.php?action=status&status=approved&id=<?= $stf['id'] ?>" class="btn btn-sm btn-success me-1" title="Approve Registration">
                                        <i class="bi bi-check-lg"></i>
                                    </a>
                                <?php endif; ?>

                                <?php if ($stfStatus !== 'disabled'): ?>
                                    <a href="staff.php?action=status&status=disabled&id=<?= $stf['id'] ?>" class="btn btn-sm btn-warning me-1" title="Disable Account">
                                ...
                                <a href="staff.php?action=delete&id=<?= $stf['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Are you sure you want to delete this staff account?');" title="Delete">
